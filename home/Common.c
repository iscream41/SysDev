...
{
    LOG(DEBUG,"psTmpToken[%d][0] = %c",g_iTokenCount, psTmpToken[g_iTokenCount][0]);
...
char* MakeToken(FILE* pFile, int iTokenCount) 
{
    ...
    //メモリ領域確保
    psMakeToken = (char*)malloc(sizeof(char) * iTokenCount);
    ...
    if( psMakeToken != NULL ) {
        ...
        LOG(DEBUG,"psMakeToken[0] Address = %p",&psMakeToken[0]);
        LOG(DEBUG,"psMakeToken Address= %p",psMakeToken);
        return psMakeToken;
