    ...
    while(1){
        ...
        //トークンが閉じ丸括弧と一致
            ...
            if(iParenCount >= 0){
                ...
                printf("括弧の対応が不正です。\n");
                LOG(WARN,"%d行目 : 括弧の対応が不正です。",g_iCodeCount);
                return 1;
            }
        }
        //トークンが代入演算子と一致
            ...
            if(i == 1) {
                ...
                if( isalpha(psToken[i-1][0]) ) {
                    ...
                    if(JudgeReserved(psToken[g_iTokenCount])) {
                        ...
                        LOG(WARN,"%d行目 : 予約語を変数名として使用しています。",g_iCodeCount);
                        return 1;
                    }
                }
                else {
                    printf("代入演算子の左辺が不正です。\n");
                    LOG(WARN,"%d行目 : 代入演算子の左辺が不正です。",g_iCodeCount);
                    return 1;
                }
            }
            else {
    ...
    if(pkLeft == NULL) {
        ...
        while(1) {
            if(psToken[g_iTokenCount][0] == '+' || psToken[g_iTokenCount][0] == '-') {
                ...
                ReleaseTree(pkLeft);
                printf("トークンの並びが不正です。\n");
                LOG(WARN,"%d行目 : トークンの並びが不正です。",g_iCodeCount);
                return NULL;
            }
        }
    }
...
TItem* Factor(char* psToken[])
{
    ...
    //不正因子
        ...
        printf("トークンの並びが不正です。\n");
        LOG(ERROR,"%d行目 : トークンの並びが不正です。",g_iCodeCount);
        return NULL;
    }
